...
values_hbf = (
    pd.read_csv(
        "transformer_A100_HBF_sim_HBF.csv",
        header=None,
        names=categories,
        index_col=None,
    )
...
# HBF
bottom = 0
for i, (category, value) in enumerate(zip(categories, values_hbf)):
    plt.bar(3, value, bottom=bottom, color=colors[i], width=0.5)
    bottom += value
sum_hbf = bottom

print(
    f"Init total (s): A100-excl={sum_a100_excl:.4f}, A100={sum_a100:.4f}, HBF={sum_hbf:.4f}"
)
print(
    f"Init speedups: excl/HBF={sum_a100_excl / sum_hbf:.3f}x, A100/HBF={sum_a100 / sum_hbf:.3f}x"
)
...
print(
    f"AR total (ms): A100-excl={sum_ar_a100_excl:.2f}, A100={sum_ar_a100:.2f}, HBF={sum_ar_hbf:.2f}"
)
print(
    f"AR speedups: excl/HBF={sum_ar_a100_excl / sum_ar_hbf:.3f}x, A100/HBF={sum_ar_a100 / sum_ar_hbf:.3f}x"
)
