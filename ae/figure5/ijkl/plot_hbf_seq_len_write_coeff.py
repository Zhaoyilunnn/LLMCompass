...
# Collect files with sequence-length tags (fixed-io-write-coeff = 20 for HBF)
# For each seq-len we collect both A100 (GPU baseline) and A100_HBF (coeff=20)
prefill_gpu = {}  # seq_len -> total_latency_s
prefill_hbf = {}  # seq_len -> total_latency_s
device_gpu = {}  # seq_len -> total_latency_ms
...
for fname in os.listdir("."):
    ...
    if fname.startswith("transformer_A100_sim_seq") and fname.endswith(".csv"):
        try:
            ...
            pass
    # Prefill A100_HBF with coeff20
    elif fname.startswith(
        "transformer_A100_HBF_sim_HBF_coeff20_seq"
    ...
    # Decode A100 baseline
        ...
        try:
            ...
            pass
    # Decode A100_HBF with coeff20
    elif fname.startswith("transformerAR_A100_HBF_sim_coeff20_seq") and fname.endswith(
        ".csv"
    ):
        try:
            tag = fname.split("_seq")[1].split(".csv")[0]
...
if prefill_keys:
    ...
    ax1.plot(xs, ys_hbf, marker="s", label="A100-HBF coeff=20")
    ax1.set_xlabel("Sequence length")
    ax1.set_ylabel("Prefill Latency (s)")
    ax1.set_title("Prefill vs Sequence Length (coeff=20)")
    ax1.grid(axis="both")
...
if decode_keys:
    ...
    ax2.plot(xs, ys_hbf, marker="s", label="A100-HBF coeff=20")
    ax2.set_xlabel("Sequence length")
    ax2.set_ylabel("Decode Latency (ms)")
    ax2.set_title("Decode vs Sequence Length (coeff=20)")
    ax2.grid(axis="both")
    for x, y_g, y_h in zip(xs, ys_gpu, ys_hbf):
        if y_g > 0:
            overhead = (y_h - y_g) / y_g
...
plt.tight_layout()
plt.savefig(
    "hbf_seq_len_prefill_decode_write_coeff.svg",
    bbox_inches="tight",
    pad_inches=0.01,
    dpi=300,
)
print("Saved hbf_seq_len_prefill_decode_write_coeff.svg")
